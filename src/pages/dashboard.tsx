...
import { useRoqPlatformUserProfiles, useEmployeeCount, useNewTableCount, useTeacheronteacheronKiCount } from 'lib/roq';
...
function DashboardPage() {
  ...
  const teacheronteacheronKiStat = useTeacheronteacheronKiCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Teacheronteacheron Kis',
      result: teacheronteacheronKiStat?.data,
      isLoading: teacheronteacheronKiStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
